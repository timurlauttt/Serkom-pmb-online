...
@section('page-description', 'Ubah informasi berita')

@section('content')
<div class="max-w-4xl">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.beritas.update', $berita->slug) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <select name="jurusan_id" id="jurusan_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            ...
                                            @foreach($jurusans as $jurusan)
                                                <option value="{{ $jurusan->id }}" {{ old('jurusan_id', $berita->jurusan_id) == $jurusan->id ? 'selected' : '' }}>{{ $jurusan->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                    <!-- Kategori -->
                    <div>
                        ...
                        <select name="category" id="category" 
                            ...
                            <option value="Event" {{ old('category', $berita->category) == 'Event' ? 'selected' : '' }}>Event</option>
                            <option value="Kegiatan" {{ old('category', $berita->category) == 'Kegiatan' ? 'selected' : '' }}>Kegiatan</option>
                            <option value="Kesiswaan" {{ old('category', $berita->category) == 'Kesiswaan' ? 'selected' : '' }}>Kesiswaan</option>
                            <option value="Kurikulum" {{ old('category', $berita->category) == 'Kurikulum' ? 'selected' : '' }}>Kurikulum</option>
                            <option value="Prestasi" {{ old('category', $berita->category) == 'Prestasi' ? 'selected' : '' }}>Prestasi</option>
                            <option value="Humas" {{ old('category', $berita->category) == 'Humas' ? 'selected' : '' }}>Humas</option>
                            <option value="Iptek" {{ old('category', $berita->category) == 'Iptek' ? 'selected' : '' }}>Iptek</option>
                        </select>
                        @error('category')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="date" name="posted_at" id="posted_at" value="{{ old('posted_at', $berita->posted_at ? $berita->posted_at->format('Y-m-d') : date('Y-m-d')) }}" 
                ...
                <div>
                    ...
                    @if($berita->image_path)
                        <div class="mb-4">
                            <img src="{{ $berita->image_url }}" alt="Current" class="w-full h-48 object-cover rounded-lg">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10" 
                              ...
                              placeholder="Tulis konten berita di sini..." required>{{ old('content', $berita->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div id="hashtagsContainer">
                        @php $hashtags = old('hashtags', $berita->hashtags ?? []); if(is_string($hashtags)) $hashtags = json_decode($hashtags, true) ?: []; @endphp
...
<script>
    ...
    // Dynamic hashtag CRUD
    function addHashtagInput(value = '') {
        const container = document.getElementById('hashtagsContainer');
        const div = document.createElement('div');
        div.className = 'flex items-center mb-2';
        div.innerHTML = `<input type="text" name="hashtags[]" value="${value}" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg mr-2" placeholder="#hashtag">
            <button type="button" class="removeHashtag px-2 py-1 bg-red-100 text-red-600 rounded">Hapus</button>`;
        container.appendChild(div);
        div.querySelector('.removeHashtag').onclick = function() { div.remove(); };
    }
    document.getElementById('addHashtag').onclick = function() { addHashtagInput(); };
    document.querySelectorAll('.removeHashtag').forEach(btn => {
