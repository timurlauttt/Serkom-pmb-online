...
@section('page-description', 'Buat data hotel baru')
        ...
        <form action="{{ route('admin.hotels.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="number" name="harga_mulai" id="harga_mulai" value="{{ old('harga_mulai') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('harga_mulai') border-red-500 @enderror" min="0">
                    @error('harga_mulai')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="text" name="kontak" id="kontak" value="{{ old('kontak') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('kontak') border-red-500 @enderror">
                    @error('kontak')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <div id="hotelImagePreview" class="mt-4 hidden">
                        <img src="" alt="Preview Gambar Hotel" class="w-full h-48 object-cover rounded-lg">
                    </div>
                    @error('gambar')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
