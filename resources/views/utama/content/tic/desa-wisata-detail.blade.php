...
@section('title', $desaWisata->nama . ' - Desa Wisata TIC SMK Taman Siswa Purwokerto')
...
<section class="section-padding" style="margin-top: 80px;">
    <div class="container" style="max-width: 1200px;">
        ...
        <p style="margin-bottom: 2rem; color: var(--text-muted);">
            ...
            <a href="{{ route('tic.index') }}#desa-wisata">Desa Wisata</a> / 
            {{ Str::limit($desaWisata->nama, 50) }}
        </p>
        ...
        <div style="display: grid; grid-template-columns: 1fr 350px; gap: 3rem;">
            ...
            <article>
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 1rem; margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 2px solid var(--border-color); align-items: center;">
                    ...
                    @if($desaWisata->jam_operasional)
                        <span style="color: var(--text-muted); display: flex; align-items: center; gap: 0.5rem; font-size: 0.9rem;">
                            <i class="fas fa-clock" style="color: var(--accent-green);"></i> 
                            {{ $desaWisata->jam_operasional }}
                        </span>
                    @endif

                    @if($desaWisata->harga_tiket)
                        <span style="background: #ffc107; color: #333; padding: 0.5rem 1rem; border-radius: 2rem; font-size: 0.875rem; font-weight: 500; display: flex; align-items: center; gap: 0.5rem;">
                            <i class="fas fa-ticket-alt"></i> Rp {{ number_format($desaWisata->harga_tiket, 0, ',', '.') }}
                        </span>
                    @endif
                </div>
                ...
                @if($desaWisata->deskripsi)
                    <div style="background: var(--bg-card); padding: 2rem; border-radius: var(--border-radius); margin-bottom: 2rem; border: 1px solid var(--border-color);">
                        ...
                        <div class="content-article" style="line-height: 1.8; font-size: 1.05rem; color: var(--text-main);">
                            {!! nl2br(e($desaWisata->deskripsi)) !!}
                        </div>
                    </div>
                @endif

                {{-- Location Information --}}
                @if($desaWisata->alamat)
                    <div style="background: var(--bg-card); padding: 2rem; border-radius: var(--border-radius); margin-bottom: 2rem; border: 1px solid var(--border-color);">
                        <h3 style="font-size: 1.5rem; font-weight: 700; color: var(--text-main); margin-bottom: 1.5rem;">
                ...
                <div style="margin-top: 2.5rem; padding-top: 2rem; border-top: 1px solid var(--border-color);">
                    ...
                    <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
                        ...
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($desaWisata->nama) }}" target="_blank"
                        ...
                        <a href="https://wa.me/?text={{ urlencode($desaWisata->nama . ' ' . url()->current()) }}" target="_blank"
            ...
            <aside>
                        ...
                        @if($desaWisata->kota)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $desaWisata->kota }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($desaWisata->jam_operasional)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $desaWisata->jam_operasional }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($desaWisata->harga_tiket)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">Rp {{ number_format($desaWisata->harga_tiket, 0, ',', '.') }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($desaWisata->alamat)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0; line-height: 1.5;">{{ $desaWisata->alamat }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                {{-- Other Desa Wisata --}}
                @php
                    $otherDesaWisata = \App\Models\Desawisata::where('id', '!=', $desaWisata->id)
                        ->orderBy('nama', 'asc')
                        ->take(5)
                        ->get();
                @endphp

                @if($otherDesaWisata->count() > 0)
                    <div style="background: var(--bg-card); padding: 2rem; border-radius: var(--border-radius); border: 1px solid var(--border-color);">
                        <h3 style="font-size: 1.25rem; font-weight: 700; color: var(--text-main); margin-bottom: 1.5rem;">Desa Wisata Lainnya</h3>
                        
                        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                            @foreach($otherDesaWisata as $other)
                                <a href="{{ route('desa-wisata-detail', $other->slug) }}" style="text-decoration: none; color: inherit; display: flex; gap: 1rem; transition: transform 0.2s;"
                                   ...
                                   onmouseout="this.style.transform='translateX(0)'">
                                    ...
                                    @else
                                        <div style="width: 80px; height: 80px; background: var(--primary-blue); border-radius: var(--border-radius); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                            <i class="fas fa-tree" style="font-size: 1.5rem; color: white;"></i>
                                        </div>
                                    @endif
