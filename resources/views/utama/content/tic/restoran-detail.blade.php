...
@section('title', $restoran->nama . ' - Restoran TIC SMK Taman Siswa Purwokerto')
...
<section class="section-padding" style="margin-top: 80px;">
    <div class="container" style="max-width: 1200px;">
        ...
        <p style="margin-bottom: 2rem; color: var(--text-muted);">
            ...
            <a href="{{ route('tic.index') }}#restoran">Restoran</a> / 
            {{ Str::limit($restoran->nama, 50) }}
        </p>
        ...
        <div style="display: grid; grid-template-columns: 1fr 350px; gap: 3rem;">
            ...
            <article>
                ...
                {{-- Featured Image --}}
                @if($restoran->gambar)
                    <img src="{{ asset($restoran->gambar) }}" alt="{{ $restoran->nama }}" 
                ...
                <div style="margin-top: 2.5rem; padding-top: 2rem; border-top: 1px solid var(--border-color);">
                    ...
                    <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
                        ...
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($restoran->nama) }}" target="_blank"
                        ...
                        <a href="https://wa.me/?text={{ urlencode($restoran->nama . ' ' . url()->current()) }}" target="_blank"
            ...
            <aside>
                        ...
                        @if($restoran->kota)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $restoran->kota }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($restoran->jam_operasional)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $restoran->jam_operasional }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($restoran->kontak)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $restoran->kontak }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($restoran->alamat)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0; line-height: 1.5;">{{ $restoran->alamat }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                {{-- Other Restorans --}}
                @php
                    $otherRestorans = \App\Models\Restoran::where('id', '!=', $restoran->id)
                        ->orderBy('nama', 'asc')
                        ->take(5)
                        ->get();
                @endphp

                @if($otherRestorans->count() > 0)
                    <div style="background: var(--bg-card); padding: 2rem; border-radius: var(--border-radius); border: 1px solid var(--border-color);">
                        <h3 style="font-size: 1.25rem; font-weight: 700; color: var(--text-main); margin-bottom: 1.5rem;">Restoran Lainnya</h3>
                        
                        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                            @foreach($otherRestorans as $otherRestoran)
                                <a href="{{ route('restoran-detail', $otherRestoran->slug) }}" style="text-decoration: none; color: inherit; display: flex; gap: 1rem; transition: transform 0.2s;"
                                   onmouseover="this.style.transform='translateX(5px)'"
                                   onmouseout="this.style.transform='translateX(0)'">
                                    @if($otherRestoran->gambar)
                                        <img src="{{ asset($otherRestoran->gambar) }}" alt="{{ $otherRestoran->nama }}"
