...
@section('title', $transportasi->nama . ' - Transportasi TIC SMK Taman Siswa Purwokerto')
...
<section class="section-padding" style="margin-top: 80px;">
    <div class="container" style="max-width: 1200px;">
        ...
        <p style="margin-bottom: 2rem; color: var(--text-muted);">
            ...
            <a href="{{ route('tic.index') }}#transportasi">Transportasi</a> / 
            {{ Str::limit($transportasi->nama, 50) }}
        </p>
        ...
        <div style="display: grid; grid-template-columns: 1fr 350px; gap: 3rem;">
            ...
            <article>
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 1rem; margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 2px solid var(--border-color); align-items: center;">
                    <span style="background: var(--primary-blue); color: white; padding: 0.5rem 1.25rem; border-radius: 2rem; font-size: 0.875rem; font-weight: 500; display: flex; align-items: center; gap: 0.5rem;">
                        <i class="fas fa-bus"></i> Transportasi
                    </span>
                    
                    @if($transportasi->kota)
                    ...
                    @if($transportasi->jenis)
                        <span style="background: var(--accent-green); color: white; padding: 0.5rem 1rem; border-radius: 2rem; font-size: 0.875rem; font-weight: 500; display: flex; align-items: center; gap: 0.5rem;">
                            <i class="fas fa-car"></i> {{ $transportasi->jenis }}
                        </span>
                    @endif

                    @if($transportasi->kontak)
                        <span style="color: var(--text-muted); display: flex; align-items: center; gap: 0.5rem; font-size: 0.9rem;">
                ...
                @if($transportasi->gambar)
                    <img src="{{ asset($transportasi->gambar) }}" alt="{{ $transportasi->nama }}" 
                ...
                <div style="margin-top: 2.5rem; padding-top: 2rem; border-top: 1px solid var(--border-color);">
                    ...
                    <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
                        ...
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($transportasi->nama) }}" target="_blank"
                        ...
                        <a href="https://wa.me/?text={{ urlencode($transportasi->nama . ' ' . url()->current()) }}" target="_blank"
            ...
            <aside>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1.25rem;">
                        @if($transportasi->kota)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $transportasi->kota }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($transportasi->jenis)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    <p style="font-size: 0.85rem; color: var(--text-muted); margin: 0 0 0.25rem 0;">Jenis</p>
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $transportasi->jenis }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($transportasi->kontak)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    <p style="font-size: 0.85rem; color: var(--text-muted); margin: 0 0 0.25rem 0;">Kontak</p>
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0;">{{ $transportasi->kontak }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($transportasi->alamat)
                            <div style="display: flex; gap: 1rem; align-items: start;">
                                ...
                                <div>
                                    ...
                                    <p style="font-size: 0.95rem; font-weight: 600; color: var(--text-main); margin: 0; line-height: 1.5;">{{ $transportasi->alamat }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                {{-- Other Transportasi --}}
                @php
                    $otherTransportasi = \App\Models\Transportasi::where('id', '!=', $transportasi->id)
                        ->orderBy('nama', 'asc')
                        ->take(5)
                        ->get();
                @endphp

                @if($otherTransportasi->count() > 0)
                    <div style="background: var(--bg-card); padding: 2rem; border-radius: var(--border-radius); border: 1px solid var(--border-color);">
                        <h3 style="font-size: 1.25rem; font-weight: 700; color: var(--text-main); margin-bottom: 1.5rem;">Transportasi Lainnya</h3>
                        
                        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                            @foreach($otherTransportasi as $other)
                                <a href="{{ route('transportasi-detail', $other->slug) }}" style="text-decoration: none; color: inherit; display: flex; gap: 1rem; transition: transform 0.2s;"
                                   ...
                                   onmouseout="this.style.transform='translateX(0)'">
                                    ...
                                    @else
                                        <div style="width: 80px; height: 80px; background: var(--primary-blue); border-radius: var(--border-radius); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                            <i class="fas fa-bus" style="font-size: 1.5rem; color: white;"></i>
                                        </div>
                                    @endif
